...
struct MessageContent {
    role: Option<String>,
    // Content may be a plain string (older format) or an array of content blocks
    // (e.g. [{type: "text", text: "..."}]) as produced by recent Claude CLI versions.
                ...
                if let Some(message) = entry.message {
                    if message.role.as_deref() == Some("user") {
                        if let Some(content_val) = message.content {
                            // Extract text from either a plain string or an array of content blocks
